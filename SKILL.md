...
### 🚫 절대 금지 (모든 출력에 무조건 적용 — 예외 없음)

1. **ASCII 박스/표 그림 절대 금지** — `┌─┬─┐` `│ │` `├─┼─┤` `└─┴─┘` `┝━┥` 같은 박스 드로잉 문자를 **단 하나도** 쓰지 말 것. 표가 필요하면 **반드시 마크다운 표** (`| 항목 | 내용 |` / `|---|---|`)만 사용. 박스 그림은 모바일·LinkedIn·복붙에서 전부 깨진다.
2. **이모지 한 응답에 5개 이하** — 섹션 헤더나 ⚠️ 경고에만.
3. **결론을 첫 줄에** — "마감됐습니다" / "1순위 충족" 등을 맨 위에.
4. **마감된 공고**: 한 줄 마감 안내 + 다음 회차 시기 + 준비 2~3개. 자격·가점 100줄 분석 금지 (사용자가 "자격 분석해줘" 명시 요청 시에만).

이 4개는 SKILL.md의 다른 어떤 규칙보다 우선한다.
...
### 4.5 출력 위생 규칙 (가독성 — 반드시 지킬 것)

**금지:**
- ❌ ASCII 박스 그림 (`┌──┬──┐`, `┝══┥` 등) — 모바일·SNS·복붙에서 깨짐
- ❌ 의사결정과 무관한 디테일 나열 (마감된 공고에 자격·가점 100줄 분석)
- ❌ 한 응답에 이모지 6개 이상 (시각적 노이즈)
- ❌ 결론을 본문 가운데·끝에 묻기

**필수:**
- ✅ **결론을 첫 줄에** — "이미 마감됐습니다" / "1순위 자격 충족" / "Top 3 추천:" 같이 한눈에
- ✅ **마크다운 표만 사용** — `| 항목 | 값 |` 형식. 박스 그림 X
- ✅ **마감·종료된 공고**: 한 줄 안내 + 다음 회차 시기 + 준비 사항 2~3개. **자격·가점 자세한 분석 X** (사용자가 "자격 분석해줘" 명시 요청 시에만)
- ✅ **이모지 절제**: 한 응답에 5개 이하. 섹션 헤더 또는 결정적 신호(⚠️ 경고)에만
- ✅ **자세한 정보는 트리거 안내로**: `ℹ️ 자세한 자격·가점 원하시면 "자격 분석" 요청`
...
ℹ️ 자격·가점 자세한 분석은 "자격 분석해줘"로
```

**나쁜 예 (지양)** — 아래 같은 출력을 절대 내지 말 것:
- ASCII 박스 드로잉 문자(`┌ ┬ ┐ │ ├ ┼ ┤ └ ┴ ┘ ━ ═` 등)로 표를 그리는 것 → **마크다운 표로 대체**
- 마감된 공고에 "자격: 무주택 ✅ / 자격: 미혼 ✅ ..." 식으로 자격·가점을 50줄+ 나열하는 것 → 한 줄 마감 안내로 대체
- 한 응답에 👤📋💰🎯⚠️✅❌ 등 이모지를 6개 이상 흩뿌리는 것
...
**결과 출력 시:**
- D-day 있는 공고 (청약홈·LH 등): 마감 임박순 정렬 — 🔴 D-1 이하 · 🟡 D-2~3 · 🟢 D-4+
- **`d_day=None`인 GH·SH 공고: 절대 누락 금지** — 일정이 PDF/HWP 안에 있어 d-day가 비지만, 응답 `count`에 포함된 공고다. 마감 임박순 정렬에선 빠지더라도 **테이블 맨 아래 별도 그룹**으로 반드시 표기:
  ```
  ⚠️ 일정 미확인 (GH·SH — 정렬 불가, 원문/분석 필요)
  | 이름 | 지역 | 유형 | 링크 |
  | 안양 에버포레 | 경기 안양 | GH 국민주택 | [원문 →](url) |
  ```
- **15줄 제한 때문에 GH·SH를 빼지 말 것** — 청약홈 공고 행을 줄여서라도 GH·SH는 최소 1줄씩 표기. "다 청약홈이야? GH 없어?" 같은 오해 방지.
- GH·SH 공고는 B 완성형 사전 워밍으로 `/v1/apt/notice/{id}/raw`가 즉시(0.5초) 응답하니, 사용자가 특정 GH·SH 공고 콕 집으면 바로 일정 분석 가능
- "마감"(d_day 음수): 목록에서 제외 (active_only=true 기본)
...
**적용 시점**: 사용자가 특정 단지에 대해 "입지 분석", "주변 시설", "역세권인지", "학군 어때", "이 동네 괜찮아?" 등을 **명시 요청**할 때.

청약 공고의 주소를 바탕으로 주변 인프라(지하철·학교·마트·병원·공원)와 지역 호재를 종합 보고한다. **외부 지도 API 도구가 필요**하며, Apify Google Maps Scraper(MCP 커넥터)가 가장 정확하다.

### 사전 설정 — Apify 커넥터 (사용자 1회 셋업)

런타임별 설치 방법이 다르지만 원리는 동일.

**Claude Desktop**:
1. https://apify.com 가입 (무료 월 $5 크레딧 ≈ 50~100건 분석)
2. Profile → Settings → API tokens 복사
3. 사용자 지정 → 커넥터 → 커넥터 둘러보기 → "Apify" 추가
...
- 도보 시간은 추정치임을 반드시 명시 ("도보 X분 (추정)" 또는 ⚠️ 라인 추가)
- 학교 등급·평판은 LLM 추정이라 "참고용" 라벨 필수
- 호재 정보는 검색 시점에 따라 달라지므로 발견 시 출처 링크 명시
- Apify 미설정 시 출력 헤더에 `(추정 보고서 — 외부 지도 도구 미사용)` 명시
- 마지막에 반드시 ⚠️ 현장 답사 권고 문구

### 비용·한계

- Apify 무료 티어: 월 $5 크레딧 → 단지 1건 분석에 ~$0.05 → 100건/월 가능
- 한 번에 너무 많은 단지 분석 요청 시: "한 번에 1개씩 처리"로 제한
- Google Maps 데이터 정확도: POI 누락·이름 변경·운영 종료 매물 가능성 존재
...
**`schedule_source: "unavailable"` 공고 처리 규칙 (중요 — PDF 자동 추출 활용):**
GH/SH 카테고리는 청약 접수기간이 PDF 안에 있습니다. announcement 응답만으로는 d_day_label이 `"⚠️ 일정 미확인 (원문 확인 필요)"`로 표시되지만, **`/v1/apt/notice/{id}/raw`를 호출하면 첨부 PDF가 자동 추출**되어 일정·자격·금액 등 모든 정보가 `text` 필드에 포함됩니다.

**필수 워크플로 (schedule_source: unavailable 공고 — 추천 전에 강제 실행):**
```
1. /v1/apt/notice/{id}/raw 호출 (force_refresh 불필요, 7일 캐시 활용)
2. 응답의 has_pdf 또는 char_count로 본문 추출 성공 여부 확인:
   - has_pdf=true → PDF 첨부에서 일정·자격 자동 추출 성공
   - has_pdf=false + char_count >= 2000 → HTML 본문에 일정 있음 (SH 케이스)
   - has_pdf=false + char_count < 1000 → 본문이 HWP 첨부에만 (GH 케이스, 자동 추출 불가)
3. text 본문에서 일정 표 파싱:
   - "청약접수", "1순위", "2순위", "당첨자 발표", "서류제출기간" 등 키워드 근처 날짜
   - "2026-04-28" / "2026.04.28" / "26.4.28" 등 다양한 포맷 지원
4. 추출한 마감일과 오늘 비교:
   - 이미 지났으면 → "❌ 마감 (2026-04-28)" 명시, 다음 회차 안내
   - 진행 중이면 → "✅ D-N (마감 2026-MM-DD)" 정확한 D-day 안내
5. 본문에서 못 찾거나 char_count<1000이면 → "⚠️ 원문 확인 필요" 안내
```
...
- `looks_blocked` 또는 `attachment_fetch_failed`가 true면 LLM은 반드시:
  ```
  ⚠️ 이 공고는 첨부파일을 자동 추출하지 못했습니다.
  (사이트 일시 차단 또는 첨부 PDF가 40MB 초과인 경우 — 청약 공고는 도면 多로 80MB+ 흔함)
  [원문 →](url)에서 직접 확인하세요.
  ```
- 자격·일정·금액 추정을 절대 하지 말고 URL 안내만 (잘못된 정보 방지)
- 첨부 추출 한계: PDF 40MB·앞 20페이지까지. 초대형 도면 PDF는 원문 안내로 처리

**처리 규칙 — PDF 추출 후에도 어떤 경우든:**
- **Top 추천 후보로 일단 보류** — PDF 본문 확인 전엔 매칭 점수만 높다고 Top에 올리지 말 것
- PDF 일정 확인 후:
  - 마감됐으면 → 별도 "📋 다음 회차 대기" 섹션 + 다음 회차 예상 시기 (GH 4·8·12월, SH 분기별)
  - 진행 중이면 → 정상 Top 추천 가능
- **D-day 색상 표시 금지** (🟢🟡🔴 대신 ⚠️만) — PDF 일정 확정해도 자동 갱신은 안 되므로 사용자에게 "PDF 기반 추정" 신호 유지
- **알림 발송 시** — reminder=d3/d1 필터에서 자동 제외됨 (proxy의 d_day가 None이므로)
- **사용자 안내 문구 예시 (PDF 일정 추출 성공 시)**:
  ```
  📋 [공고명] (GH 청년 매입임대)
   PDF 본문 확인 결과: 1순위 청약 2026-04-28 ❌ 마감
   다음 회차 예상: 2026년 8월경 (GH 분기별 모집)
   [원문 →](url)
  ```
- **PDF 추출 실패 시 (has_pdf=false 또는 일정 파싱 실패)**:
  ```
  📋 일정 미확인 공고 — 직접 확인 필요
   1. [공고명] · [지역] · 등록 X일 전 · [원문 →](url)
   ⚠️ PDF 자동 추출 실패. 이미 마감됐을 수 있으니 클릭해서 확인하세요.
  ```
...
- **GH·SH 별도 그룹 필수**: `d_day=None`인 GH·SH 공고가 `count`에 있으면 위 예시처럼 "⚠️ 일정 미확인" 표를 **반드시 추가**. 0건이면 생략. 누락 절대 금지.
