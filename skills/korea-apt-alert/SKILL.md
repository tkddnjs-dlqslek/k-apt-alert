...
description: 한국 청약 공고를 개인 프로필 기반으로 조회·분석하고 Slack/Telegram으로 알림을 보낸다. 공공데이터포털 6개 API를 프록시 경유로 통합 조회하며, 사용자 API 키 없이 동작한다. 출력에 ASCII 박스 그림(┌─┐│└─┘) 절대 사용 금지 — 표는 항상 마크다운(| 항목 | 값 |)으로만.
license: MIT
metadata:
...
**데이터 접근 우선순위 — MCP 툴 우선 (플러그인 설치 시), curl 폴백**
이 스킬이 `k-apt-alert` 플러그인으로 설치돼 MCP 툴이 로드돼 있으면, 아래 curl 대신 **MCP 툴을 우선 사용**한다. 같은 프록시를 부르지만 타입 정의·인코딩이 안전하다.

| 작업 | MCP 툴 | curl 폴백 엔드포인트 |
|---|---|---|
| 공고 조회 | `search_announcements` (category·region·district·active_only·months_back·min_units·constructor_contains·reminder) | `GET /v1/apt/announcements` |
| 카테고리 목록 | `list_categories` | `GET /v1/apt/categories` |
| 가점·자격·1순위 | `score_profile` (profile·specials·announcements) | `POST /v1/apt/score` |
| 모집공고 원문 | `get_notice_raw` (notice_id·url·force_refresh) | `GET /v1/apt/notice/{id}/raw` |
| 공고 변동 | `get_changes` (since·change_type·limit) | `GET /v1/apt/changes` |
| 알림 발송 | `send_notification` | `POST /v1/apt/notify` |
| 경쟁률 | `get_competition` (notice_id·history) | `GET /v1/apt/announcements/{id}/competition` |

- 응답 구조는 MCP·curl 동일하므로 아래 출력 규칙은 그대로 적용.
- MCP 툴이 없으면(수동 스킬 설치·Codex 등 MCP 미연결 환경) 아래 curl 폴백을 그대로 사용한다.
- 한국어 파라미터 퍼센트 인코딩 이슈는 MCP 툴을 쓰면 자동 해결됨 (curl 폴백 시에만 아래 인코딩 규칙 적용).
...
| 통장 | 무주택 | 가능한 청약 |
|---|---|---|
| ✅ 있음 | ✅ 무주택 | APT 일반분양, LH 공공분양 + 특별공급 자격 확인 |
| ✅ 있음 | ❌ 1주택+ | 갈아타기 일부 조건부 + 오피스텔·잔여세대·임의공급 (무주택 불문) |
| ❌ 없음 | (무관) | 오피스텔/도시형, 잔여세대, 임의공급 (선착순) |
