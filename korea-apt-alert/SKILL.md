...
| # | 선택 | 결과 (어떻게 도착하는지) | 이런 분께 추천 |
|---|------|----------------------|-----------------|
| 1 | 지금 한 번만 | 바로 지금 Slack/Telegram으로 공고 1회 도착하고 끝 | "오늘만 확인해볼게요" |
| 2 | 매일 자동으로 (GitHub 서버 이용) ⭐ | 매일 아침 지정 시각에 공고 자동 도착. 내 PC가 꺼져있어도 계속 동작 | "꾸준히 받고 싶어요" — **가장 안정적** |
| 3 | 내 PC가 켜져있을 때 매일 | 매일 지정 시각에 내 PC가 직접 알림 발송. PC 꺼진 시간엔 쉼 | "데스크톱 항상 켜둬요" |
| 4 | 지금 Claude 창 열어둔 동안만 반복 | Claude Code 대화창이 열려있는 동안 N시간마다 자동 반복 | "잠깐 테스트해볼게요" |

1~4 중 선택해주세요. (2번 추천)

ℹ️ 설정 방법 안내 (기술 용어 참고)
  1번 = 즉시 API 호출 (설정 불필요)
  2번 = GitHub Actions 워크플로 (레포에 yaml 파일 1개 + Secrets 1~2개 등록, 처음 5분)
  3번 = cron (macOS/Linux) 또는 Windows 작업 스케줄러
  4번 = Claude Code의 /loop 커맨드
```
